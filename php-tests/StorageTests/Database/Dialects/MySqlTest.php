...
class MySqlTest extends CommonTestClass
{
    ...
    public function testSelect(): void
    {
        ...
        $query->setLimits(5,3);
        $sql = new Dialects\MySQL();
        $this->assertEquals('SELECT `foo`.`bar` AS `baz`, `foo`.`htf` AS `yjd`, `dfg`.`vrs` AS `abh` FROM `foo`  LEFT JOIN `btr` AS `dfg` ON (`foo`.`fds` = `dfg`.`gda`)  WHERE `foo`.`dbt` <=> :dbt_0 AND `foo`.`dfd` != :dfd_1 AND `dfg`.`bcf` IS NOT NULL  GROUP BY `foo`.`gds` ORDER BY `dfg`.`vrs` ASC LIMIT 5,3;', $sql->select($query));
        $this->assertEquals([ ':dbt_0' => 'ggf', ':dfd_1' => 'yxn', ], $query->getParams());
        $query->resetCounter();
    }
}
